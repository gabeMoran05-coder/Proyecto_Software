...
def enviar_ticket_por_whatsapp(request, venta, telefono, enviar_audio=False):
    _validar_configuracion()
    ...
    if enviar_audio:
        ...
        audio_mp3 = _generar_audio_ticket_mp3(audio_texto)
...
def _generar_audio_ticket_mp3(texto):
    if settings.OPENAI_API_KEY:
        return _generar_audio_ia(texto)

    try:
        from gtts import gTTS
    except ImportError as exc:
        raise WhatsAppIntegrationError(
            'Para enviar audio configura OPENAI_API_KEY o instala gTTS en requirements.txt.'
        ) from exc
    ...
    buffer = BytesIO()
    try:
        gTTS(text=texto, lang='es', tld='com.mx').write_to_fp(buffer)
    except Exception as exc:
        raise WhatsAppIntegrationError(
            'No se pudo generar el audio MP3. Revisa la conexión del servidor o configura OPENAI_API_KEY.'
        ) from exc
