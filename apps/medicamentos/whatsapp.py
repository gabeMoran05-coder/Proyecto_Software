...
def _api_error(nombre, response):
    ...
    if nombre.startswith('WhatsApp') and response.status_code == 401:
        return (
            'WhatsApp rechazo la autenticacion. Revisa que WHATSAPP_ACCESS_TOKEN '
            'este vigente y pertenezca al mismo numero configurado en '
            'WHATSAPP_PHONE_NUMBER_ID. Mientras lo actualizas, puedes usar '
            'el boton "Abrir WhatsApp Web" para enviar el mensaje manualmente.'
        )
